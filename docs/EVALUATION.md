...
## 三层测试架构（节点/链路/性能）重构结果

### ① 节点测试（T24）：36/36 = 100%
- 6节点单节点直测（coordinator/knowledge/experience/interview/document/chat）
- 大量Redis缓存命中（0.02s），验证缓存生效
- 文件: scripts/eval/node_tests.py

### ② 链路测试（T25）：4/5 = 80%
| 场景 | 结果 | 说明 |
|------|------|------|
| 沉淀经验 | ✅ | 走skill路径，trace 0步（缺陷5）|
| 知识库RAG | ✅ | trace 2步，15.5s |
| 面试弱题→学习计划 | ✅ | 1个Redis计划 |
| 跨会话记忆 | ❌ | 问数据库选型被路由到knowledge查库，未走记忆召回（意图路由问题）|
| MCP外部工具 | ✅ | trace 2步，15.3s |

### ③ 性能测试（T26）
| 指标 | 结果 |
|------|------|
| 并发读20 | 20/20，总0.12s，平均111ms |
| 并发写5(含LLM) | 5/5，总11.85s |
| Redis未命中/命中 | 49.4s / 20.71s（缺陷4）|
| Token统计 | 受影响（缺陷3）|
...
| # | 修复方式 | 验证 |
|---|----------|------|
| 2 | 意图识别提示词优化（区分文档/知识库/记忆回顾）| 待复测 |
| 3 | _record_execution加tokens参数，节点内捕获usage | coordinator已记录 |
| 4 | 缓存key去掉history | 函数级 5.4s→0.00s ✅ |
| 5 | skill路径加AgentExecution记录 | 已记录 |

**函数级缓存验证**: 意图识别 5.4s→0.00s | chat_completion 7.9s→0.00s ✅
...
**结论**: 明确意图下温度对识别无影响（系统鲁棒）。结构化任务建议低温度，创意任务（文档生成）可用高温度。
...
**结论**: 历史窗口越大 prompt token 越多（成本↑），需权衡上下文完整性与 token 成本。

### 并发测试（真实链路）
- 5并发聊天（含LLM）: 5/5 成功，总0.4s（Redis缓存命中）
- 缺陷5修复验证: skill路径现在有trace
